...
from regularizers import ortho_l2_reg
...
def _conv_bn_relu(**conv_params):
    ...
    """
    global layer, ortho_l2_glb
    filters = conv_params["filters"]
    kernel_size = conv_params["kernel_size"]
    strides = conv_params.setdefault("strides", (1, 1))
    kernel_initializer = conv_params.setdefault("kernel_initializer", "he_normal")
    padding = conv_params.setdefault("padding", "same")
    kernel_regularizer = conv_params.setdefault("kernel_regularizer", ortho_l2_reg(*ortho_l2_glb))
...
def _shortcut(input, residual):
    ...
    if stride_width > 1 or stride_height > 1 or not equal_channels:
        shortcut = Conv2D(filters=residual_shape[CHANNEL_AXIS],
                          ...
                          kernel_regularizer=ortho_l2_reg(*ortho_l2_glb), name='layer{}/conv-s'.format(layer))(input)
    ...
    def f(input):
        ...
        if is_first_block_of_first_layer:
            ...
            conv1 = Conv2D(filters=filters, kernel_size=(3, 3),
                           ...
                           padding="same",
                           kernel_initializer="he_normal",
                           kernel_regularizer=ortho_l2_reg(*ortho_l2_glb), name='layer{}/conv'.format(layer))(input)
...
def init_obs(ortho_l2):
    global layer
    global ortho_l2_glb
    ortho_l2_glb = ortho_l2
    layer = 1
...
class ResnetBuilder(object):
    ...
    def build(input_shape, num_outputs,
              block_fn, repetitions, ortho_l2,
              name='model', hiddens=512, last_act_layer='softmax'):
        ...
        dense = Dense(units=1024, kernel_initializer='he_normal',
                      kernel_regularizer=ortho_l2_reg(*ortho_l2_glb),
...
class ResNet(BaseModel):
    ...
    def __init__(self, input_shape, classes, config, ortho_l2,
                 with_bn=True, with_dp=True, hiddens=512,
                 last_act_layer='softmax'):
        super(ResNet, self).__init__(input_shape,
                                     classes, config,
                                     with_bn, with_dp,
                                     hiddens, last_act_layer)
