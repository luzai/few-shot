        ...
        for tensor_name in self.tensors_names:
            for e in self.em.Tensors(tensor_name):
                ...
                if (now_step, tensor_name) not in tensors:
                    ...
                    tensors[(now_step, tensor_name)] = np.concatenate((now_tensor, tensors[(now_step, tensor_name)]),
                                                                      axis=0)
...
class Loader(threading.Thread):
    def __init__(self, name=None, path=None, parallel=True, stat_only=False):  #
